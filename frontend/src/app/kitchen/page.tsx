...
const NEXT_BTN: Record<string,string> = { 
  pending: 'bg-sky-600 hover:bg-sky-500 text-white shadow-md shadow-sky-900/40', 
  confirmed: 'bg-orange-600 hover:bg-orange-500 text-white shadow-md shadow-orange-900/40', 
  preparing: 'bg-emerald-600 hover:bg-emerald-500 text-white shadow-md shadow-emerald-900/40', 
  ready: 'bg-stone-600 hover:bg-stone-500 text-white shadow-md' 
};
...
export default function KitchenPage() {
  ...
  return (
    <div className="min-h-screen bg-stone-900 text-stone-100">
      {/* Header */}
      <header className="sticky top-0 z-40 bg-stone-950 border-b border-stone-800 px-4 py-3 shadow-lg">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            <span className="text-2xl">👨‍🍳</span>
            <div>
              <h1 className="font-bold text-base text-white">Kitchen Display</h1>
          ...
          <div className="flex gap-2">
            {[
              { label:'New', count:counts.pending, color:'bg-amber-400' },
              { label:'Cooking', count:counts.confirmedPreparing, color:'bg-orange-400' },
              { label:'Ready', count:counts.ready, color:'bg-emerald-400' },
            ].map(s=>(
              <div key={s.label} className="flex items-center gap-1.5 px-3 py-1.5 bg-stone-800 border border-stone-700 rounded-full text-xs">
                <span className={`w-2 h-2 rounded-full ${s.color}`}/>
                <span className="text-stone-200 hidden sm:inline">{s.label}</span>
        ...
        <div className="flex gap-2">
          {([['active',`Active (${counts.pending+counts.confirmedPreparing})`],['ready',`Ready (${counts.ready})`],['all','All']] as const).map(([id,label])=>(
            <button key={id} onClick={()=>setFilter(id)}
              className={cn('px-4 py-1.5 rounded-full text-xs font-semibold transition-all border', filter===id?'bg-brand-500 text-white border-brand-600':'bg-stone-800 text-stone-200 border-stone-700 hover:bg-stone-700')}>
              {label}
            </button>
          ))}
          <button onClick={fetchOrders} className="ml-auto px-3 py-1.5 rounded-full text-xs font-bold bg-stone-800 text-stone-200 border border-stone-700 hover:bg-stone-700 transition-all">↻ Refresh</button>
        </div>
      </header>
      ...
      <main className="p-4">
        {loading ? (
          <div className="flex justify-center py-20"><Spinner size="lg" className="text-stone-300"/></div>
        ) : sorted.length === 0 ? (
          <div className="text-center py-20">
            <span className="text-6xl block mb-4">🍽️</span>
            <p className="text-stone-300 text-lg font-medium">No {filter === 'all' ? '' : filter} orders</p>
            <p className="text-stone-400 text-sm mt-1">New orders will appear here instantly</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
            <AnimatePresence>
...
function KitchenOrderCard({ order, onAdvance, onDetail, updating }: any) {
  ...
  const statusColors: Record<string, string> = {
    pending: 'bg-amber-950 text-amber-300 border border-amber-800/60',
    confirmed: 'bg-sky-950 text-sky-300 border border-sky-800/60',
    preparing: 'bg-orange-950 text-orange-300 border border-orange-800/60',
    ready: 'bg-emerald-950 text-emerald-300 border border-emerald-800/60'
  };

  return (
    <motion.div layout initial={{opacity:0,scale:0.9,y:20}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.9}} transition={{duration:0.2}}
      className={cn('bg-stone-950 border border-stone-800 rounded-2xl border-t-4 overflow-hidden cursor-pointer hover:ring-2 hover:ring-stone-500 transition-all shadow-md',
        STATUS_BORDER[order.status]||'border-t-stone-600',
        urgent?'ring-2 ring-red-500 bg-stone-900':'')}
      onClick={onDetail}>
      {/* Header */}
      <div className="px-4 py-3 border-b border-stone-800 bg-stone-900/50">
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2">
              <p className="font-black text-white text-xl">T{order.tableNumber}</p>
              {order.priority==='urgent'&&<span className="bg-red-600 text-white text-xs font-black px-2 py-0.5 rounded-full animate-pulse shadow">URGENT</span>}
            </div>
            <p className="text-xs text-stone-300 font-mono mt-0.5">{order.orderNumber}</p>
          </div>
          <div className="text-right">
        ...
        {order.customerName&&<p className="text-xs text-stone-200 font-medium mt-2 flex items-center gap-1"><span>👤</span> {order.customerName}</p>}
      </div>

      {/* Items */}
      <div className="px-4 py-3 space-y-2.5 min-h-[90px] bg-stone-950">
        {order.items?.slice(0,5).map((item:any,i:number)=>(
          <div key={i} className="flex items-start gap-2.5">
            <span className="bg-brand-500 text-white text-xs font-black w-5 h-5 rounded-md flex items-center justify-center flex-shrink-0 mt-0.5">{item.quantity}</span>
            <div className="flex-1 min-w-0">
              <p className="text-sm text-stone-50 font-semibold truncate leading-tight">{item.name}</p>
              {item.notes&&<p className="text-xs text-amber-300 font-medium mt-0.5 bg-amber-950/60 px-1.5 py-0.5 rounded border border-amber-900/40">📝 {item.notes}</p>}
            </div>
          </div>
        ))}
        {order.items?.length>5&&<p className="text-xs text-stone-300 font-semibold pt-1">+{order.items.length-5} more items...</p>}
        {order.notes&&<div className="mt-2 p-2 bg-stone-900 border border-stone-800 rounded-lg"><p className="text-xs text-amber-200 font-medium">📋 {order.notes}</p></div>}
      </div>

      {/* Footer */}
      <div className="px-4 py-2 bg-stone-900/40 border-t border-stone-800 flex justify-between items-center">
        <span className="text-xs text-stone-300 font-medium">{order.items?.length} item{order.items?.length!==1?'s':''}</span>
        <span className="text-sm font-black text-white">{formatCurrency(order.total)}</span>
      </div>

      {/* Action button */}
      {next && (
        <div className="px-4 pb-4 pt-2 bg-stone-950" onClick={e=>e.stopPropagation()}>
          <button onClick={onAdvance} disabled={updating}
            className={cn('w-full h-11 rounded-xl font-bold text-sm text-white transition-all active:scale-[.98] flex items-center justify-center gap-2',
...
function KitchenOrderDetail({ order, onAdvance, updating }: any) {
  ...
  return (
    <div className="overflow-y-auto max-h-[80vh] bg-stone-900 text-stone-100">
      <div className="p-5 space-y-4">
        {/* Status */}
        <div className={cn('rounded-2xl p-4 text-center border-2',
        ...
        {/* All items — full detail for kitchen */}
        <div>
          <p className="text-xs font-bold text-stone-300 uppercase tracking-wider mb-2">Order Items ({order.items?.length})</p>
          <div className="space-y-2">
            {order.items?.map((item:any,i:number)=>(
              <div key={i} className="bg-stone-950 border border-stone-800 rounded-xl p-3 flex items-start gap-3">
                {item.image&&<img src={item.image} alt={item.name} className="w-14 h-14 rounded-lg object-cover flex-shrink-0 border border-stone-800"/>}
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <p className="text-white font-bold text-base truncate">{item.name}</p>
                    <span className="bg-brand-500 text-white text-xs font-black px-2.5 py-0.5 rounded-md">×{item.quantity}</span>
                  </div>
                  {item.notes&&<p className="text-amber-300 text-xs font-medium mt-1.5 bg-amber-950/80 border border-amber-900/60 rounded-lg px-2.5 py-1">📝 {item.notes}</p>}
                  <p className="text-stone-300 text-xs mt-1.5 font-medium">{formatCurrency(item.price)} each</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {order.notes&&(
          <div className="bg-amber-950/40 border border-amber-500/40 rounded-xl p-3">
            <p className="text-xs font-bold text-amber-400 mb-1 uppercase tracking-wider">📋 Order Notes</p>
            <p className="text-amber-100 text-sm font-semibold">{order.notes}</p>
          </div>
        )}
